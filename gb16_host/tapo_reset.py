...
def _group_creds(group_id: int) -> tuple[str, str]:
    """그룹별 계정이 지정되면 우선 사용, 없으면 공통 계정 사용."""
    email = os.getenv(f'TAPO_GROUP_{group_id}_EMAIL', '').strip() or TAPO_EMAIL
    pw    = os.getenv(f'TAPO_GROUP_{group_id}_PASSWORD', '').strip() or TAPO_PASSWORD
    return email, pw

GROUP_NAMES: dict[int, str] = {
    1: 'IF + Bellshaker + Light1',
    2: 'Domino',
    3: 'IF Ceiling + Light2',
}
...
async def reset_group(group_id: int, off_sec: int = OFF_SECONDS) -> bool:
    ...
    email, password = _group_creds(group_id)
